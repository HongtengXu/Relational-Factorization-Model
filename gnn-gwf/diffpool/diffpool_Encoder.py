...
from diffpool.set2set import Set2Set
...
class SoftPoolingGcnEncoder(GcnEncoderGraph):
    ...
    def forward(self, x, adj, batch_num_nodes, assign_input):
        x_a = assign_input
        ...
        final_embed = self.pred_model(output)
        return final_embed[0]
    ...
    def get_embeddings(self, dataloader, mask_nodes=True):
        embeddings = []
        ys = []
        ...
        with torch.no_grad():
            for data in dataloader_tmp:
                ...
                x = Variable(data['feats'].float(), requires_grad=False).to(device)
                y = Variable(data['label'], requires_grad=False).to(device)
                ...
                embeddings.append(final_embed[0].cpu().numpy())
