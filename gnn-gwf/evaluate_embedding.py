...
def svc_classify(x, y, search):
    ...
    for train_index, test_index in kf.split(x, y):
        ...
        if search:
            ...
            classifier = GridSearchCV(SVC(gamma="auto"), params, cv=5, scoring='accuracy', verbose=0)
        else:
...
def evaluate_embedding(embeddings, labels, search=True):
    ...
    logreg_accuracies, logreg_std = logistic_classify(x, y)
    # print(logreg_accuracies)
    print('LogReg', np.mean(logreg_accuracies), np.mean(logreg_std))

    svc_accuracies, svc_std = svc_classify(x,y, search)
    # print(svc_accuracies)
    print('svc', np.mean(svc_accuracies), np.mean(svc_std))

    linearsvc_accuracies, linearsvc_std = linearsvc_classify(x, y, search)
    # print(linearsvc_accuracies)
    print('LinearSvc', np.mean(linearsvc_accuracies), np.mean(linearsvc_std))

    randomforest_accuracies, randomforest_std = randomforest_classify(x, y, search)
    # print(randomforest_accuracies)
    print('randomforest', np.mean(randomforest_accuracies), np.mean(randomforest_std))
