...
class FGWF(nn.Module):
    ...
    def __init__(self,
        ...
        if prior is None:
          ...
          .detach().numpy()      prob = torch.ones(num_nodes, 1)/num_nodes
                if data.x is None:
    ...
    def fgwBarycenter(self,
             ...
             trans: List,
             idx: int,
             idx_embedding: torch.Tensor,
             idx_edge_index: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """
...
if __name__ == '__main__':
    with warnings.catch_warnings():
        ...
        index_samples = list(range(len(dataset)))
        ...
        for epoch in range(1, epochs+1):
            ...
            for idx in tqdm(index_samples):
                ...
                data_tmp = dataset[idx] # another copy of the data, get_adjacency will modify data_tmp in place
                data_tmp.edge_attr = None # we do not take account of edge_attr, set it to 0; otherwise, get_adjacency will give wrong adjacency matrix
                graph = get_adjacency(data_tmp)['adj']
                num_nodes = graph.size(0)
                ...
                graph_b, embedding_b, weights_k = model.fgwBarycenter(prob, trans, idx, emb, edge_index)
