...
for mode in ['directed', 'undirected']:
    ...
    for nn in range(NN):
        ...
        trans3, d_gw3, _ = GWL.gromov_wasserstein_discrepancy(cost_s, cost_t, p_s, p_t, ot_dict)
        t3 = time.time()

        print(len(d_gw1), len(d_gw2), len(d_gw3))
        dgw_badmm[:len(d_gw1), nn] = np.asarray(d_gw1)
        dgw_badmm[len(d_gw1):, nn] = d_gw1[-1]
        dgw_ppa[:len(d_gw2), nn] = np.asarray(d_gw2)
        dgw_ppa[len(d_gw2):, nn] = d_gw2[-1]
        dgw_entropy[:len(d_gw3), nn] = np.asarray(d_gw3)
        dgw_entropy[len(d_gw3):, nn] = d_gw3[-1]

        print('Sparsity:\nb-admm={:.1f} time={:.3f}sec,\nippa={:.1f} time={:.3f}sec,\nentropy={:.1f} time={:.3f}sec\n'.format(
            np.sum(trans1 == 0) / trans1.size * 100, t1 - t0,
            np.sum(trans2 == 0) / trans2.size * 100, t2 - t1,
            np.sum(trans3 == 0) / trans3.size * 100, t3 - t2))
    ...
    dgw_entropy_mean = np.mean(dgw_entropy, axis=1)
    dgw_entropy_std = 0.5 * np.std(dgw_entropy, axis=1)

    plt.figure(figsize=(5, 5))
    plt.semilogx(range(MM), dgw_ppa_mean, label='PGA', color='blue')
    plt.fill_between(range(MM), dgw_ppa_mean - dgw_ppa_std, dgw_ppa_mean + dgw_ppa_std,
                     color='blue', alpha=0.2)
    plt.semilogx(range(MM), dgw_entropy_mean, label='Entropy', color='red')
    plt.fill_between(range(MM), dgw_entropy_mean - dgw_entropy_std, dgw_entropy_mean + dgw_entropy_std,
                     color='red', alpha=0.2)
    plt.semilogx(range(MM), dgw_badmm_mean, label='B-ADMM', color='orange')
